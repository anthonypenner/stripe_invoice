    ...
    def perform
      ...
      sicharges.each do |charge|
        ...
        data = {
          ...
          bt: Stripe::BalanceTransaction.retrieve(charge.indifferent_charge_json[:balance_transaction]),
          owner: owner,
        }
        puts "[DJTaxReport] aggregating data #{charge.stripe_id}"
        if charge.indifferent_charge_json[:refunds].count > 0
          arr_refunds = []
          charge.indifferent_charge_json[:refunds].each do |refund|
            refd = {
              refund: refund,
